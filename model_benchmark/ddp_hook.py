...
QUANT_FAC = 1e5
CODEC = nvcomp.Codec(algorithm="GDeflate")
...
def noop_hook(state, bucket):
    return bucket.buffer()


class RegisterHook(TrainerCallback):
    """
    Register DDP hook for gradient compression.
    """
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        if hasattr(model, "module"):
            ddp_module = model.module
        else:
            ddp_module = model
        if not hasattr(ddp_module, "register_comm_hook"):
            print("Model is not DDP, skipping hook registration.")
            return
        print("Registering DDP communication hook.")
        """
        model.register_comm_hook(state=None, hook=noop_hook)
